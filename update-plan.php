...
add_action('wp_ajax_rnl_quotes_activate_hit_to_update_plan', 'rnl_quotes_activate_hit_to_update_plan');
add_action('wp_ajax_nopriv_rnl_quotes_activate_hit_to_update_plan', 'rnl_quotes_activate_hit_to_update_plan');

/**
 * Activate R+L LTL
 */
function rnl_quotes_activate_hit_to_update_plan($network_wide = null)
{
    if ( is_multisite() && $network_wide ) {
        ...
        foreach (get_sites(['fields'=>'ids']) as $blog_id) {
            ...
            $license_key = get_option('wc_settings_rnl_plugin_licence_key');
            strlen($license_key) > 0 ? $post_data['license_key'] = $license_key : '';

            $url = RNL_FREIGHT_DOMAIN_HITTING_URL . "/web-hooks/subscription-plans/create-plugin-webhook.php?";
            ...
            en_check_rnl_plan_on_product_detail();
 ...
 */
function en_check_rnl_plan_on_product_detail()
{
    ...
    $nested_mateials = apply_filters('rnl_quotes_quotes_plans_suscription_and_features', 'nested_material');
...
//  Dropship
    if (get_option('rnl_quotes_store_type') == "1") {
        $action_dropship = apply_filters('rnl_quotes_quotes_plans_suscription_and_features', 'multi_dropship');
    ...
    update_option('eniture_plugin_14', array('rnl_quotes_packages_quotes_package' => array('plugin_name' => 'LTL Freight Quotes - R+L Edition', 'multi_dropship' => $dropship_feature_PD, 'hazardous_material' => $hazardous_feature_PD, 'nested_material' => $nested_material_PD)));
}

/**
 * Deactivate R+L LTL
 */
function rnl_quotes_deactivate_hit_to_update_plan($network_wide = null)
{
    if ( is_multisite() && $network_wide ) {
 ...
 * @return string
 */
function rnl_quotes_plan_name()
{

    $plan = get_option('rnl_quotes_packages_quotes_package');
    $expire_days = get_option('rnl_package_expire_days');
    $expiry_date = get_option('rnl_freight_package_expire_date');
    $plan_name = "";
...
/**
 * Show R+L LTL Plan Notice
 * @return string
 */
function rnl_quotes_plan_notice()
{

    if (isset($_GET['tab']) && ($_GET['tab'] == "rnl_quotes")) {
        $plan_number = get_option('rnl_quotes_packages_quotes_package');
        $store_type = get_option('rnl_quotes_store_type');

        if ($store_type == "1" || $store_type == "0" && ($plan_number == "0" || $plan_number == "1" || $plan_number == "2" || $plan_number == "3")) {

            $click_here_to_update_plan = ' <a href="javascript:void(0)" data-action="rnl_quotes_activate_hit_to_update_plan" onclick="en_update_plan(this);">Click here</a> to refresh the plan';
            $plan_package = rnl_quotes_plan_name();
...
add_action('admin_notices', 'rnl_quotes_plan_notice');
