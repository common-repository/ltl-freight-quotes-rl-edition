...
if (!class_exists('EnOrderExport')) {
    ...
    class EnOrderExport
    {
        ...
        public function en_order_details_by_id($order, $order_id, $current_date = '')
        {
            ...
            $order_created_date = isset($current_date) && strlen($current_date) > 0 ? $current_date : (isset($order->order_date) ? $order->order_date : '');
        ...
        public function en_orders_sending($orders, $en_orders_ids)
        {
            ...
            $post_data = [
                'serverName' => rnl_quotes_get_domain(),
                'licenseKey' => get_option('wc_settings_rnl_plugin_licence_key'),
                'currencyUnit' => get_option('woocommerce_currency'),
                'platform' => 'wordpress',
                'orders' => $orders,
            ];

            $curl_obj = new RNL_Curl_Request();
            $response = $curl_obj->rnl_get_curl_response($url, $post_data);
